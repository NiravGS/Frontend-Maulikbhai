...
import { Row, Col, List, Avatar, Radio, Form, Card, Rate, message, Badge } from "antd";
...
const TripsPublicView = (props) => {
  ...
  console.log('SIMILAR :: ', similarTrip);
        ...
        <div className="container align-center">
          <div className="expedition_bg">
                                ...
                                {trip.dateTime.map((d, i) => {
                                  if (i === 0 || i === 1 || i === 2) {
                                    return (<Radio.Button key={d._id} value={i}
                                      onClick={() => selectDate(d.from, d.price)}>
                                      {moment(d.from).format("LL")}
                                    </Radio.Button>)
                                  } else {
                                    return null;
                                  }
            ...
            <Row gutter={20} className="mt40 mb40">
              ...
              <Col span={8}>
                {trip.exclusion && (<div className="trip_inclusion_sec trip_detail_des">
                  <h4 className="medium-text an-18 pb30">Exclusion</h4>
              ...
              <Col>
                <div className="photo_sec">
                  ...
                  <Row gutter={40} className="pb40">
                    {similarTrip.length > 1 && (similarTrip.map((l) => {
                      if (l.id !== id) {
                        return (
                          <Col span={8} key={l.id} className="gutter-row">
                            <Card
                              onClick={() => props.history.push(`/trips-details/${l.id}`)}
                              className="price_sec"
                              hoverable
                              cover={<img alt="example" src={l.cover} />}>
                              <div className="price_sec">
                                <h5 className=" an-14 medium-text">{l._doc.title}</h5>
                                <p className="mb10">Activity : {l._doc.activity.toString()}</p>
                                <Row>
                                  <Col span={18}>
                                    <h3 className="medium-text an-18">{l._doc.price} $</h3>
                                  </Col>
                                      ...
                                      {colors.map((color) => (
                                        <div key={color}>
                                          <Badge
                                            ...
                                            text={
                                              parseInt(l.skill) < 50 ?
                                                "Light" : parseInt(l._doc.skill) >= 50 && parseInt(l._doc.skill) < 100 ?
                                                  "Medium" : "Hard"
                                            }
                                            className="an-14"
                                          />
